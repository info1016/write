...
<title>정보과 세특 생성기 V10 (디자인 복원 및 API 수정)</title>
...
<header>
  <div class="logo" id="logo" title="3번 클릭 시 관리자 모드 진입">세특<span>생성기</span> <small style="font-size: 10px; opacity: 0.4;">V10</small></div>
...
<div class="app-container">
  ...
  <div class="result-area">
    <div class="keyword-input-wrap">
      ...
      <input type="text" id="studentKeyword" placeholder="이 학생만의 개별 특징 (예: '보고서의 논리 구조가 탄탄함', '모둠 활동 시 조원들을 배려함')">
    </div>
    <div class="result-header">
      <div class="result-text-label">NEIS 세특 생성 문구 미리보기</div>
    ...
    <textarea id="generatedText" spellcheck="false" placeholder="항목 체크 후 AI 자동 창작을 누르세요."></textarea>
  </div>
</div>
...
<script>
  ...
  const defaultRubrics = [
    { title: "개념 이해", items: [{label: "변수와 조건문의 원리 이해"}, {label: "알고리즘 설계 능력 우수"}] },
    { title: "문제 해결 및 실습", items: [{label: "스스로 오류를 분석하고 디버깅함"}, {label: "창의적인 기능 추가"}] },
    { title: "태도 및 소양", items: [{label: "수업 집중도가 높고 끈기 있음"}, {label: "정보 윤리를 준수함"}] }
  ];

  // ── 초기화 ──
  window.addEventListener('DOMContentLoaded', () => {
    const savedStudents = localStorage.getItem('eval_v10_students');
    studentsData = savedStudents ? JSON.parse(savedStudents) : {"1":[], "2":[], "3":[]};
    const savedRubrics = localStorage.getItem('eval_v10_rubrics');
    rubricData = savedRubrics ? JSON.parse(savedRubrics) : JSON.parse(JSON.stringify(defaultRubrics));
    geminiApiKey = localStorage.getItem('eval_v10_api') || "";
  ...
  // ── 핵심: Gemini AI 생성 (v1 정식 버전으로 수정) ──
  document.getElementById('aiGenerateBtn').onclick = async () => {
    if (activeStudentIdx === null) return alert('학생을 먼저 선택하세요.');
    if (!geminiApiKey) return alert('관리자 모드에서 API 키를 먼저 입력해주세요.');
    saveState();
    const sels = studentsData[currentClass][activeStudentIdx].eval || [];
    if (sels.length === 0) return alert('항목을 체크해주세요.');

    const keyword = document.getElementById('studentKeyword').value.trim();
    const prompt = `너는 중학교 정보교사야. 다음 항목으로 학생의 생활기록부 세특을 작성해줘.
    항목: ${sels.join(', ')} / 개별특징: ${keyword || '없음'}.
    조건: 2-3문장, '~함', '~임'으로 끝낼 것. 설명 제외 본문만 출력.`;

    const area = document.getElementById('generatedText');
    area.value = "AI가 작성 중입니다..."; area.classList.add('loading');

    try {
      // API 엔드포인트를 v1 정식 버전으로 변경하여 에러 수정
      ...
      if (!res.ok) throw new Error(result.error ? result.error.message : 'API 호출 실패');

      const aiText = result.candidates[0].content.parts[0].text.trim();
      area.value = aiText; area.classList.remove('loading');
      studentsData[currentClass][activeStudentIdx].text = aiText;
    } catch (e) {
      area.classList.remove('loading');
      area.value = "⚠️ 오류 발생: " + e.message;
    }
  };
  ...
  function saveToLocal() { localStorage.setItem('eval_v10_students', JSON.stringify(studentsData)); localStorage.setItem('eval_v10_rubrics', JSON.stringify(rubricData)); localStorage.setItem('eval_v10_api', geminiApiKey); }
  ...
  document.getElementById('exportBtn').onclick = () => {
    ...
    const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], {type:'text/csv'})); link.download = `세특_결과.csv`; link.click();
  };
  ...
  document.getElementById('adminSaveAllBtn').onclick = () => {
    ...
    alert('설정이 저장되었습니다.'); document.getElementById('adminPanel').style.display = 'none';
  };
  ...
  function renderRubricEditor() {
    ...
    rubricData.forEach(cat => {
      ...
      div.innerHTML = `<input type="text" class="cat-title" value="${cat.title}" style="font-weight:bold; width:70%; padding:5px;"> <button class="btn btn-danger btn-sm" onclick="this.parentElement.remove()">삭제</button>` + 
  ...
  window.addItem = (btn) => { const div = document.createElement('div'); div.style="display:flex; gap:5px; margin-bottom:5px;"; div.innerHTML=`<input type="text" class="item-label" placeholder="항목명" style="flex:1; padding:5px;"> <button onclick="this.parentElement.remove()">X</button>`; btn.previousElementSibling.appendChild(div); };
  window.addCategory = () => { const div = document.createElement('div'); div.className='edit-category'; div.innerHTML=`<input type="text" class="cat-title" placeholder="주제명" style="font-weight:bold; width:70%; padding:5px;"> <button onclick="this.parentElement.remove()">삭제</button><div class="item-list" style="margin-top:10px;"></div><button onclick="addItem(this)">+ 항목 추가</button>`; document.getElementById('rubricEditorContainer').appendChild(div); };
</script>
</body>
</html>
